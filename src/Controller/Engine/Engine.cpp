...
auto Engine::run() -> void {
    ...
    engine.scene = std::make_shared<Scene>();
...
auto Engine::processInput() -> void {
    ...
    while (gleqNextEvent(&event)) {
        ...
        engine.scene->handleInputData(inputManager.ProcessInput(event));
        gleqFreeEvent(&event);
    }
    if (!handledMouse) {
...
bool BlueEngine::Engine::getMouseMode() const {
    auto mouseMode = glfwGetInputMode(window, GLFW_CURSOR);
